...
class DateTestCase(GDOTestCase):
    ...
    async def test_db_date_is_ok(self):
        date = Time.get_date()
    ...
    async def test_tz_and_display(self):
        # TZ Parser human
        ...
        self.assertEqual('11/09/1980 13:37', displayed, 'Berlin Date can be printed as UTC')
    ...
    async def test_value_conversion(self):
    ...
    async def test_microseconds(self):
        ...
        self.assertEqual('499µs', h, 'Microseconds do not work in GDT_Duration.')
        h = Time.human_duration(0.001499)
        self.assertEqual('1ms 499µs', h, 'Microseconds do not work #2.')
        h = GDT_Duration('d').units(2, True).value(0.001499).render()
        self.assertEqual('1ms 499µs', h, 'Microseconds do not work in GDT_Duration #2.')
